-- ================================================================
-- CARTELERA DIGITAL — ESQUEMA COMPLETO (TABLAS + RLS + STORAGE)
-- Ejecutar TODO junto en SQL Editor de Supabase:
-- https://supabase.com/dashboard/project/wntecetvtwsmylsxexgt/sql/new
-- ================================================================
...
-- ============================================================
-- 7. ROW LEVEL SECURITY — TODAS LAS POLITICAS (UNIFICADO)
-- ============================================================
...
-- ── DEVICES (usuario panel + dispositivo anonimo) ────────────
...
DROP POLICY IF EXISTS "Usuarios insertan sus dispositivos" ON public.devices;
...
DROP POLICY IF EXISTS "Usuarios eliminan sus dispositivos" ON public.devices;
CREATE POLICY "Usuarios eliminan sus dispositivos" ON public.devices
    FOR DELETE USING (auth.uid() = user_id);

-- Politicas adicionales para dispositivo anonimo (app TV)
DROP POLICY IF EXISTS "Pairing: find device by key" ON public.devices;
CREATE POLICY "Pairing: find device by key" ON public.devices
    FOR SELECT USING (auth.role() = 'authenticated');
DROP POLICY IF EXISTS "Device self-update" ON public.devices;
CREATE POLICY "Device self-update" ON public.devices
    FOR UPDATE USING (auth.role() = 'authenticated');

-- ── MEDIA (usuario panel + dispositivo anonimo) ──────────────
ALTER TABLE public.media ENABLE ROW LEVEL SECURITY;
DROP POLICY IF EXISTS "Usuarios ven su media" ON public.media;
CREATE POLICY "Usuarios ven su media" ON public.media
    FOR SELECT USING (auth.uid() = user_id);
DROP POLICY IF EXISTS "Usuarios insertan su media" ON public.media;
CREATE POLICY "Usuarios insertan su media" ON public.media
    FOR INSERT WITH CHECK (auth.uid() = user_id);
DROP POLICY IF EXISTS "Usuarios actualizan su media" ON public.media;
CREATE POLICY "Usuarios actualizan su media" ON public.media
    FOR UPDATE USING (auth.uid() = user_id);
DROP POLICY IF EXISTS "Usuarios eliminan su media" ON public.media;
CREATE POLICY "Usuarios eliminan su media" ON public.media
    FOR DELETE USING (auth.uid() = user_id);
DROP POLICY IF EXISTS "Device can read media" ON public.media;
CREATE POLICY "Device can read media" ON public.media
    FOR SELECT USING (auth.role() = 'authenticated');

-- ── PLAYLISTS (usuario panel + dispositivo anonimo) ──────────
ALTER TABLE public.playlists ENABLE ROW LEVEL SECURITY;
DROP POLICY IF EXISTS "Usuarios ven sus playlists" ON public.playlists;
CREATE POLICY "Usuarios ven sus playlists" ON public.playlists
    FOR SELECT USING (auth.uid() = user_id);
DROP POLICY IF EXISTS "Usuarios insertan sus playlists" ON public.playlists;
CREATE POLICY "Usuarios insertan sus playlists" ON public.playlists
    FOR INSERT WITH CHECK (auth.uid() = user_id);
DROP POLICY IF EXISTS "Usuarios actualizan sus playlists" ON public.playlists;
CREATE POLICY "Usuarios actualizan sus playlists" ON public.playlists
    FOR UPDATE USING (auth.uid() = user_id);
DROP POLICY IF EXISTS "Usuarios eliminan sus playlists" ON public.playlists;
CREATE POLICY "Usuarios eliminan sus playlists" ON public.playlists
    FOR DELETE USING (auth.uid() = user_id);
DROP POLICY IF EXISTS "Device can read playlists" ON public.playlists;
CREATE POLICY "Device can read playlists" ON public.playlists
    FOR SELECT USING (auth.role() = 'authenticated');

-- ── SCHEDULES (usuario panel + dispositivo anonimo) ──────────
ALTER TABLE public.schedules ENABLE ROW LEVEL SECURITY;
DROP POLICY IF EXISTS "Usuarios ven sus schedules" ON public.schedules;
CREATE POLICY "Usuarios ven sus schedules" ON public.schedules
    FOR SELECT USING (auth.uid() = user_id);
DROP POLICY IF EXISTS "Usuarios insertan sus schedules" ON public.schedules;
CREATE POLICY "Usuarios insertan sus schedules" ON public.schedules
    FOR INSERT WITH CHECK (auth.uid() = user_id);
DROP POLICY IF EXISTS "Usuarios actualizan sus schedules" ON public.schedules;
CREATE POLICY "Usuarios actualizan sus schedules" ON public.schedules
    FOR UPDATE USING (auth.uid() = user_id);
DROP POLICY IF EXISTS "Usuarios eliminan sus schedules" ON public.schedules;
CREATE POLICY "Usuarios eliminan sus schedules" ON public.schedules
    FOR DELETE USING (auth.uid() = user_id);
DROP POLICY IF EXISTS "Device can read schedules" ON public.schedules;
CREATE POLICY "Device can read schedules" ON public.schedules
    FOR SELECT USING (auth.role() = 'authenticated');

-- ── DEVICE LOGS (usuario panel + dispositivo anonimo) ────────
ALTER TABLE public.device_logs ENABLE ROW LEVEL SECURITY;
DROP POLICY IF EXISTS "Usuarios ven logs de sus dispositivos" ON public.device_logs;
...
DROP POLICY IF EXISTS "Device insert own logs" ON public.device_logs;
CREATE POLICY "Device insert own logs" ON public.device_logs
    FOR INSERT WITH CHECK (
        auth.role() = 'authenticated'
    );
DROP POLICY IF EXISTS "Device read own logs" ON public.device_logs;
...
-- Politicas de Storage — media
DROP POLICY IF EXISTS "Public can read media" ON storage.objects;
CREATE POLICY "Public can read media" ON storage.objects
    FOR SELECT USING (bucket_id = 'media');
DROP POLICY IF EXISTS "Auth can insert media" ON storage.objects;
CREATE POLICY "Auth can insert media" ON storage.objects
    FOR INSERT WITH CHECK (bucket_id = 'media' AND auth.role() = 'authenticated');
DROP POLICY IF EXISTS "Auth can delete own media" ON storage.objects;
CREATE POLICY "Auth can delete own media" ON storage.objects
    FOR DELETE USING (bucket_id = 'media' AND auth.role() = 'authenticated');

-- Politicas de Storage — thumbnails
...
DROP POLICY IF EXISTS "Auth can insert thumbnails" ON storage.objects;
CREATE POLICY "Auth can insert thumbnails" ON storage.objects
...
DROP POLICY IF EXISTS "Auth can insert avatars" ON storage.objects;
CREATE POLICY "Auth can insert avatars" ON storage.objects
    FOR INSERT WITH CHECK (bucket_id = 'avatars' AND auth.role() = 'authenticated');

-- ============================================================
-- 9. FUNCION AUXILIAR: Schedule activo para un dispositivo
-- ============================================================
